...
@section('content')
    <div class="container mx-auto px-4 py-8">
        ...
        <form action="{{ route('employees.update', $employee) }}" method="POST" class="bg-white rounded-lg shadow p-6">
            @csrf
            @method('PUT')
                ...
                <div>
                    ...
                    <select name="employee_type" id="employee_type"
                        ...
                        <option value="teacher"
                            {{ old('employee_type', $employee->employee_type) == 'teacher' ? 'selected' : '' }}>Teacher
                        </option>
                        <option value="staff"
                            {{ old('employee_type', $employee->employee_type) == 'staff' ? 'selected' : '' }}>Staff
                        </option>
                    </select>
                    @error('employee_type')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
